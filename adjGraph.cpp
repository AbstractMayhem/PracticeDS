...
void Graph::insertEdge(int V1, int V2, int W) {
  if (V1 < nov && V2 < nov)
    ...
    cout << "Either of " << V1 << " or " << V2 << " is greater than " << nov
         << endl;
}
...
void Graph::dijkastraAlgo(int srcVertex) {
  ...
  // Assign distance value zero to src vertexff
  dist[srcVertex] = 0;

  /*
   * This while Loop will exit untill all vertex are visited in shortest path
   * set: a.Picking a Vertex u which has minimum distance value and is not in
   * sptSet. b.Include u to sptSet. c.Update distance value of all adjacent
   * vertices of u.To update the distance values,iterate through all adjacent
   * vertices.For every adjacent vertex v, if sum of distance value of u (from
