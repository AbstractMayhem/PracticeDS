...
public:
  Graph(int);
  void insertGraph(int V1,
                   int V2); // Enter the two vertices which have edge between
                            // them edge would be V1->V2(Directed)
...
void Graph::deleteEdge(int V1, int V2) {
  ...
  while (ptr != NULL && ptr->info != V2) {
    prevNode = ptr;
    ptr = ptr->next;
  }
  if (ptr != NULL && ptr->info == V2) {
    deleteNode = ptr;
    if (prevNode == NULL)
      graph[V1] = NULL;
    else
    ...
    cout << "Edge " << V1 << " " << V2 << " does not exists!!!." << endl;
  }
}
